...
export const useTemplateConfig = () => {
  ...
  const reduxToken = useSelector(
    (state: any) => state.auth?.userDetails?.token,
  );
  ...
  const fetchTemplateConfigs = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        'http://localhost:5000/api/template-configs',
        {
          method: 'GET',
          headers: {
  ...
  const saveTemplateConfig = async (configData) => {
    ...
    try {
      const response = await fetch(
        'http://localhost:5000/api/save-template-config',
        {
          method: 'POST',
          headers: {
            Authorization: `Bearer ${authToken}`,
  ...
  const setDefaultTemplate = async (templateId) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:5000/api/template-configs/${templateId}/set-default`,
        {
          method: 'PUT',
          headers: {
            Authorization: `Bearer ${authToken}`,
  ...
  const deleteTemplateConfig = async (templateId) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:5000/api/template-configs/${templateId}`,
        {
          method: 'DELETE',
          headers: {
            Authorization: `Bearer ${authToken}`,
