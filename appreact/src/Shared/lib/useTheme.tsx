...
const useTheme = () => {
  ...
  const design = useSelector(
    (state: RootState) => state?.settings?.mySystemOfDesign,
  );
  const selectedDesignName = useSelector(
    (state: RootState) => state?.settings?.settings?.designName,
  );

  useEffect(() => {
    const foundDesign = design?.find((p: any) => p.name === selectedDesignName);
  ...
  const createGradient = (colors: any, angle = 135) => {
        ...
        paper:
          themeMode === 'dark'
            ? selectedDesign?.backgroundColor
              ? `color-mix(in srgb, ${
                  (selectedDesign as any).backgroundColor
                } 80%, white 20%)`
              : '#1f2937'
            : '#ffffff',
      },
        ...
        styleOverrides: {
          ...
          contained: {
            background: selectedDesign?.gradientColors
              ? createGradient((selectedDesign as any).gradientColors)
              : selectedDesign?.primaryColor || '#667eea',
            color: selectedDesign?.textColor || '#ffffff',
            '&:hover': {
              background: selectedDesign?.gradientColors
                ? createGradient((selectedDesign as any).gradientColors, 45)
