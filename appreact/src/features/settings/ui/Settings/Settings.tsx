...
const HeaderSection = styled(Box)(({ theme }) => ({
  ...
  padding: '24px',
  background:
    (theme as any).customTheme?.gradient ||
...
const TemplatePreview = styled(Box)<TemplatePreviewProps>(
...
const Settings = () => {
  ...
  const templateInvoice = useSelector(
    (state: any) => state?.settings?.settings?.templateInvoice,
  );
  ...
  useEffect(() => {
    setSelectedTemplate(templateInvoice || 'basicInput');
  }, [templateInvoice]);
