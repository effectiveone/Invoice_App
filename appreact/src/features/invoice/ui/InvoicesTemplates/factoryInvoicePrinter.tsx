...
const FactoryInvoicePrinter = () => {
  ...
  const designSettings = useSelector((state: any) => state?.settings?.settings);
  const designSystems = useSelector(
    (state: any) => state?.settings?.mySystemOfDesign,
  );
  const currentDesign = designSystems?.find(
    (design: any) => design.name === designSettings?.designName,
  );
  ...
  const renderConfigurableTemplate = (templateId: string) => {
